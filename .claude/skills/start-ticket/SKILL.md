...
Notion 도구 prefix는 환경마다 다르다 (예: `mcp__notion-local`, `mcp__notion-desktop`).  
아래 STEP 실행 전 ToolSearch로 `notion retrieve` 검색해 사용 가능한 도구를 확인한다.  
...
```
API-query-data-source 로 database_id 조회
...
Notion MCP 도구로 해당 티켓 페이지를 조회한다 (`API-retrieve-a-page`). 아래 항목을 파악한다:
...
Notion MCP 도구로 Phase 1 기획문서 페이지를 조회한다 (`API-retrieve-a-page`).  
페이지 ID는 `README.md` → Notion 문서 섹션에서 확인한다.

필요 시 `API-get-block-children` 으로 하위 블록까지 읽어 티켓과 관련된 기능 설명·UX 흐름·제약사항을 파악한다.

## STEP 4: 작업 계획 초안 작성

티켓 내용과 기획문서를 종합해 아래 구조의 작업 계획을 초안한다.
...
1. `API-patch-block-children` 으로 티켓 페이지 본문에 계획 추가
2. `API-patch-page` 으로 Status → `In Progress` 변경
