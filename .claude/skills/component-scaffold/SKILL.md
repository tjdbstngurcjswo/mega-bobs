...
| **Hook**              | `src/hooks/<name>.ts`          | `useVote.ts`                  |
...
├── ComponentName.constants.ts    ← 전용 상수 (필요 시)
└── ComponentName.geometry.ts     ← 순수 계산 함수, React 미사용 (필요 시)
```

서브 컴포넌트(외부 미노출)는 `_ParentNameSubName` 형식으로 둔다.
**이름 규칙**: `_` + 상위 컴포넌트명 + 서브 컴포넌트명 (예: `MenuBoard` → `_MenuBoardEmpty`).
**파일 vs 폴더**: 서브 컴포넌트에 types나 styles 파일이 필요하면 폴더로, 단순 JSX만이면 `.tsx` 파일로.
