...
4. **엔지니어 리뷰** — `code-review` 스킬을 `Skill` 도구로 호출한다 (effort: medium).
   - 버그·성능·보안·설계 관점에서 분석. 수정이 필요한 이슈는 수정 후 `fix(review): <설명>` 커밋.
   - 수정 불필요 소견(wontfix)은 PR 본문 "주요 리뷰 요청사항"에 기재한다.

모든 검사 통과 시 "✅ 코드 리뷰 통과" 보고 후 STEP 2 진행.
