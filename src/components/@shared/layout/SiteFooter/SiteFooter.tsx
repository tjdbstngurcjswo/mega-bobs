    ...
    <div className="mx-auto w-[min(880px,calc(100%-40px))] py-6">
      <div className="flex items-center justify-between gap-x-6 gap-y-3 max-[560px]:flex-col max-[560px]:items-center">
        <div className="flex items-center gap-5 max-[560px]:flex-col max-[560px]:items-center max-[560px]:gap-1">
          <span className="text-ink text-[13px] font-extrabold">MegaBobs</span>
        ...
        <div className="flex items-center gap-5">
          {FOOTER_LINKS.map((l) => (
            <Link
              key={l.label}
              href={l.href}
              className="text-muted hover:text-ink-2 text-[12px] font-medium max-[560px]:hidden"
            >
              {l.label}
            </Link>
          ))}
          <span className="text-muted text-[12px]">© 2026 megabobs</span>
        </div>
      </div>
    </div>
  </footer>
